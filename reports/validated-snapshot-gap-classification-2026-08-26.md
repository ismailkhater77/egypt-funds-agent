...
The Source-Coverage Gap report initially contained **31** funds with no validated snapshot. The official Banque du Caire Al Wefak feed was subsequently integrated and inserted a valid 26-Aug-2026 snapshot, reducing the live gap to **30**.
...
| Blank `price_update_url` in the catalog | 29 | 28 within the remaining uncovered set | This is a metadata count, **not** a claim that no official source exists |
...
The collector **does reject future valuation dates**. This is intentional: a future “next update” or expected dealing date must never become a historical validated NAV. For weekly funds, a day without a new valuation is classified as `no_new_valuation`, not as a source failure. The policy accepts a weekly NAV when the source publishes an actual date on or before the as-of date; it does not accept a future date merely because the fund’s dealing cycle is weekly.
